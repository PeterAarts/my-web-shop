      ...
      <div class="card-body">
        <form @submit.prevent="saveAllSettings">
          ...
          <div v-show="activeTab === 'payments'">
            ...
            <div v-else>
              <div v-for="provider in paymentProviders" :key="provider._id" class="mb-4 p-3 border rounded">
                <div class="row align-items-top">
                  ...
                  <div class="col-md-9">
                    <form @submit.prevent="savePaymentProvider(provider)">
                      <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" role="switch" :id="'payment-enable-' + provider._id" v-model="provider.isEnabled">
                        <label class="form-check-label" :for="'payment-enable-' + provider._id">Enable {{ provider.name }}</label>
                      </div>

                      <div v-if="provider.isOnline" class="row">
                        <div class="mb-3 col-2">
                          <label :for="'mode-' + provider._id" class="form-label small text-muted">Mode</label>
                          <select class="form-select form-select-sm" :id="'mode-' + provider._id" v-model="provider.mode">
                            <option value="sandbox">Sandbox</option>
                            <option value="live">Live</option>
                          </select>
                        </div>
                        <div class="mb-3 col-5">
                          <label :for="'clientId-' + provider._id" class="form-label small text-muted">Client ID</label>
                          <input type="text" class="form-control" :id="'clientId-' + provider._id" v-model="provider.credentials.clientId">
                        </div>
                        <div class="mb-3 col-5">
                          <label :for="'clientSecret-' + provider._id" class="form-label small text-muted">Client Secret</label>
                          <input type="password" class="form-control" :id="'clientSecret-' + provider._id" v-model="provider.credentials.clientSecret" placeholder="••••••••••••••">
                      ...
                      <div v-else>
                        <div class="mb-3">
                          <label :for="'instructions-' + provider._id" class="form-label small text-muted">Payment Instructions</label>
                          <textarea class="form-control" :id="'instructions-' + provider._id" rows="4" v-model="provider.instructions" placeholder="Enter bank account details..."></textarea>
                        </div>
                      </div>
                      
                      <button type="submit" class="btn btn-secondary btn-sm">Save {{ provider.name }}</button>
                    </form>
                  </div>
                </div>     
              </div>     
